...
// DDMacroTracker functions.

DDMacroTracker::DDMacroTracker(llvm::SmallSet<std::string, 4> &Ignore,
                                       std::vector<CallbackCall> &CallbackCalls,
                                       clang::Preprocessor &PP,
                                       clang::CompilerInstance &Instance)
    : CallbackCalls(CallbackCalls), Ignore(Ignore), PP(PP), Instance(Instance) {}

DDMacroTracker::~DDMacroTracker() {}

// Callback functions.

// Callback invoked whenever a source file is entered or exited.
void DDMacroTracker::FileChanged(
...
void
DDMacroTracker::FileSkipped(const clang::FileEntry &SkippedFile,
...
bool
DDMacroTracker::FileNotFound(llvm::StringRef FileName,
...
void DDMacroTracker::InclusionDirective(
  ...
  appendArgument("Imported", Imported);
	...
	clang::FileID fd = Instance.getSourceManager().getMainFileID();
	if (fd.isInvalid()) {
		return;
	}            
	const clang::FileEntry *fe = Instance.getSourceManager().getFileEntryForID(fd);
	std::string mainFileName = fe ->getName();
  appendArgument("MainFile", mainFileName);
}

// Callback invoked whenever there was an explicit module-import
// syntax.
void DDMacroTracker::moduleImport(clang::SourceLocation ImportLoc,
...
void DDMacroTracker::EndOfMainFile() { /*beginCallback("EndOfMainFile");*/ }

// Callback invoked when a #ident or #sccs directive is read.
void DDMacroTracker::Ident(clang::SourceLocation Loc, llvm::StringRef Str) {
  // beginCallback("Ident");
  // appendArgument("Loc", Loc);
...
void
DDMacroTracker::PragmaDirective(clang::SourceLocation Loc,
...
void DDMacroTracker::PragmaComment(clang::SourceLocation Loc,
...
// read.
void DDMacroTracker::PragmaDetectMismatch(clang::SourceLocation Loc,
                                              llvm::StringRef Name,
...
void DDMacroTracker::PragmaDebug(clang::SourceLocation Loc,
...
void DDMacroTracker::PragmaMessage(
...
// is read.
void DDMacroTracker::PragmaDiagnosticPush(clang::SourceLocation Loc,
...
// is read.
void DDMacroTracker::PragmaDiagnosticPop(clang::SourceLocation Loc,
                                             llvm::StringRef Namespace) {
...
void DDMacroTracker::PragmaDiagnostic(clang::SourceLocation Loc,
                                          llvm::StringRef Namespace,
...
void DDMacroTracker::PragmaOpenCLExtension(
...
void DDMacroTracker::PragmaWarning(clang::SourceLocation Loc,
...
void DDMacroTracker::PragmaWarningPush(clang::SourceLocation Loc,
                                           int Level) {
...
void DDMacroTracker::PragmaWarningPop(clang::SourceLocation Loc) {
...
void
DDMacroTracker::MacroExpands(const clang::Token &MacroNameTok,
...
void
DDMacroTracker::MacroDefined(const clang::Token &MacroNameTok,
...
void DDMacroTracker::MacroUndefined(const clang::Token &MacroNameTok,
                      const clang::MacroDefinition &MacroDefinition,
...
void DDMacroTracker::Defined(const clang::Token &MacroNameTok,
                                 const clang::MacroDefinition &MacroDefinition,
                                 clang::SourceRange Range) {
...
void DDMacroTracker::SourceRangeSkipped(clang::SourceRange Range) {
  // beginCallback("SourceRangeSkipped");
  // appendArgument("Range", Range);
}

// Hook called whenever an #if is seen.
void DDMacroTracker::If(clang::SourceLocation Loc,
...
void DDMacroTracker::Elif(clang::SourceLocation Loc,
                              clang::SourceRange ConditionRange,
                              ConditionValueKind ConditionValue,
...
void DDMacroTracker::Ifdef(clang::SourceLocation Loc,
...
void DDMacroTracker::Ifndef(clang::SourceLocation Loc,
                                const clang::Token &MacroNameTok,
                                const clang::MacroDefinition &MacroDefinition) {
...
void DDMacroTracker::Else(clang::SourceLocation Loc,
                              clang::SourceLocation IfLoc) {
  // beginCallback("Else");
  // appendArgument("Loc", Loc);
  // appendArgument("IfLoc", IfLoc);
...
void DDMacroTracker::Endif(clang::SourceLocation Loc,
                               clang::SourceLocation IfLoc) {
  // beginCallback("Endif");
  // appendArgument("Loc", Loc);
  // appendArgument("IfLoc", IfLoc);
...
void DDMacroTracker::beginCallback(const char *Name) {
...
void DDMacroTracker::appendArgument(const char *Name, bool Value) {
  appendArgument(Name, (Value ? "true" : "false"));
}

// Append an int argument to the top trace item.
void DDMacroTracker::appendArgument(const char *Name, int Value) {
  std::string Str;
  llvm::raw_string_ostream SS(Str);
  SS << Value;
...
void DDMacroTracker::appendArgument(const char *Name, const char *Value) {
  if (DisableTrace)
    return;
...
void DDMacroTracker::appendArgument(const char *Name,
                                        llvm::StringRef Value) {
...
void DDMacroTracker::appendArgument(const char *Name, clang::FileID Value) {
  if (Value.isInvalid()) {
...
void DDMacroTracker::appendQuotedArgument(const char *Name,
                                              const std::string &Value) {
  std::string Str;
  llvm::raw_string_ostream SS(Str);
  SS << "\"" << Value << "\"";
...
void DDMacroTracker::appendFilePathArgument(const char *Name,
                                                llvm::StringRef Value) {
...
llvm::StringRef
DDMacroTracker::getSourceString(clang::CharSourceRange Range) {
